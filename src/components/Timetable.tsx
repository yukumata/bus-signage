  ...
  return (
    <div className="departures">
      {upcoming.map((dep, index) => (
        <div key={index} className={`departure-item ${index === 0 ? "highlight" : ""}`}>
          <span className="departure-time">{dep.time.slice(0, 2)}:{dep.time.slice(2)}</span>
    ...
    const parsePublicBusXml = (xmlDoc: Document): PublicBusTimetable => {
        ...
        for (const stop of stops) {
            ...
            for (const route of routes) {
                ...
                const weekdays = route.getElementsByTagName('Weekday')[0]?.getElementsByTagName('Departure');
                if (weekdays) schedule.Weekday = Array.from(weekdays).map(d => ({ time: d.getAttribute('time') || '', note: d.getAttribute('note') || '' }));

                const saturdays = route.getElementsByTagName('Saturday')[0]?.getElementsByTagName('Departure');
                if (saturdays) schedule.Saturday = Array.from(saturdays).map(d => ({ time: d.getAttribute('time') || '', note: d.getAttribute('note') || '' }));

                const holidays = route.getElementsByTagName('Holiday')[0]?.getElementsByTagName('Departure');
                if (holidays) schedule.Holiday = Array.from(holidays).map(d => ({ time: d.getAttribute('time') || '', note: d.getAttribute('note') || '' }));
